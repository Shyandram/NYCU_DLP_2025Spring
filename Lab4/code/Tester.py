...
class Test_model(VAE_Model):
    ...
    def eval(self):
        ...
        for idx, (img, label) in enumerate(tqdm(val_loader, ncols=80)):
            ...
            self.writer = SummaryWriter(os.path.join(self.args.save_root, f'seq{idx}'))
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(1, 630):
            ...
            psnr = Generate_PSNR(pred.cpu(), img[0].cpu())
            total_psnr += psnr
            self.writer.add_scalar(f'test/PSNR', psnr, i)
        print(f"PSNR: {total_psnr/629:.4f}")
