...
class MaskGIT:
    ...
    # @staticmethod
    def prepare(self):
        os.makedirs(os.path.join(self.mask_func, "test_results"), exist_ok=True)
        os.makedirs(os.path.join(self.mask_func, "mask_scheduling"), exist_ok=True)
        os.makedirs(os.path.join(self.mask_func, "imga"), exist_ok=True)
    ...
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                ...
                z_indices_predict, mask_bc = self.model.inpainting(z_indices_predict, mask_bc, ratio, mask_num, self.mask_func)  #mask_bc: mask in latent domain
            ...
            vutils.save_image(dec_img_ori, os.path.join(self.mask_func, "test_results", f"image_{i:03d}.png"), nrow=1) 

            #demo score 
            vutils.save_image(maska, os.path.join(self.mask_func, "mask_scheduling", f"test_{i}.png"), nrow=10) 
            vutils.save_image(imga, os.path.join(self.mask_func, "imga", f"test_{i}.png"), nrow=7)
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='transformer_checkpoints\ckpt_92.pt', help='load ckpt')
