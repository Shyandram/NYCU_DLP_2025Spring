...
def init_weights(m, gain=1.0, bias_const=0.0):
...
class Actor(nn.Module):
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        logstds = self.log_std_out(x)
        log_std = torch.clamp(logstds, self.log_std_min, self.log_std_max)
...
class PPOAgent:
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.float64, bool]:
        ...
        next_state, reward, terminated, truncated, _ = self.env.step(action.squeeze(0))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save", type=str, default="task2-3")
    ...
    agent = PPOAgent(env, args)
    model_str = "100"
    agent.actor.load_state_dict(torch.load("LAB7_413551036_task2_ppo_pendulum.pth"))
    agent.test(video_folder="videos_Task2")